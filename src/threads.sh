...
this->init(){ local namespace=$1;
    new_f $this->scriptLocation/"sharedmemory.sh" this->__sharedMemory__ "" 1 "$namespace"
}
...
this->runThread2(){ local name=$1; local threadFunction=$2;
    this->__sharedMemory__->setVar "__threads.$name.done" 0
    (
        $threadFunction
        
        #set a flag to indicate that the thread has finished
        this->__sharedMemory__->setVar "__threads.$name.done" 1
    ) &

    local pid=$!
    this->__sharedMemory__->setVar "__threads.$name.pid" $pid
    this->__sharedMemory__->setVar "__threads.byPid.$pid" "$name"
...
this->killThread_byName(){ local name=$1;
    local pid=$(this->__sharedMemory__->getVarC "__threads.$name.pid")
    if [ -z "$pid" ]; then
...
this->killThread_byPid(){ local pid=$1;
    local name=$(this->__sharedMemory__->getVarC "__threads.byPid.$pid")
...
this->waitThread_byName(){ local name=$1; local timeout=$2;
    local doneFlag=$(this->__sharedMemory__->getVarC "__threads.$name.done")
    ...
    while [ "$doneFlag" -ne "1" ]; do
        ...
        sleep 0.1
        doneFlag=$(this->__sharedMemory__->getVarC "__threads.$name.done")
    done
